...
def test_public_spot_interactions_and_owner_delete(client: TestClient, database: pymysql.Connection, tmp_path, session_headers) -> None:
    ...
    try:
        created = client.post("/api/v1/spots", headers=session_headers(1), json=SPOT_BODY)
        spot_id = created.json()["id"]
        assert len(client.get("/api/v1/spots").json()) == 1
        ...
        assert client.put(f"/api/v1/spots/{spot_id}/reactions/like", headers=session_headers(1)).json()["likeCount"] == 1
        assert client.put(f"/api/v1/spots/{spot_id}/reactions/bookmark", headers=session_headers(1)).json()["active"] is True
        public = client.get("/api/v1/spots", headers=session_headers(1)).json()[0]
        assert public["isLiked"] and public["isBookmarked"] and public["isOwner"]
        comment = client.post(f"/api/v1/spots/{spot_id}/comments", headers=session_headers(1), json={"content": "야경이 정말 좋아요."}).json()
        assert comment["moderationStatus"] == 1
        assert len(client.get(f"/api/v1/spots/{spot_id}/comments").json()) == 1
        assert len(client.get(f"/api/v1/spots/{spot_id}/comments", headers=session_headers(1)).json()) == 1
        ...
        assert len(client.get(f"/api/v1/spots/{spot_id}/comments").json()) == 1
        assert client.delete(f"/api/v1/spots/{spot_id}", headers=session_headers(2)).status_code == 403
        assert client.delete(f"/api/v1/spots/{spot_id}", headers=session_headers(1)).status_code == 204
    finally:
