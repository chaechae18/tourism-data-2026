...
function SpotCard({ bookmarked, comments, onBookmark, onComment, onLike, rank, spot }) {
  ...
  return (
    <article className="border border-[#e6ddd2] bg-white p-4">
    ...
    <section className="space-y-7">
      ...
      <div>
        ...
        <div className="space-y-2">
          {spots.filter((spot) => spot.user === user.nickname).map((spot) => <div key={spot.id} className="flex items-center gap-3 border border-[#e6ddd2] bg-white p-3"><MapPin size={17} className="text-[#b8661c]" /><div className="min-w-0"><p className="truncate font-bold text-[#241b16]">{spot.place}</p><p className="mt-0.5 truncate text-xs text-[#7c6d61]">{spot.review}</p></div></div>)}
