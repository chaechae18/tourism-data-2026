...
## 위치 검증 생략 정책 (v2-2026-09-20)

사용자가 위치 검증을 생략하기로 확정했습니다. 현재 API의 `demoCompletion: true`를 통한 위치 없는 완료는 허용된 제품 동작으로 평가합니다. 기존 RT-10은 미해결 취약점에서 승인된 정책으로 전환합니다. 이는 보안 결함을 수정한 건수와 구분합니다.

로그인 여부, 본인 코스 소유권, 동일 퀘스트 재전송·동시 요청의 보상 중복 방지는 계속 검사합니다. 위치 확인 실패를 공격 성공으로 판정하던 항목은 위치 없는 완료의 정상 동작 및 비로그인 차단 검사로 교체했습니다. 앱의 일반 위치 기반 완료 경로를 삭제한 변경은 아닙니다.
