...
- 시나리오 버전: v1-2026-09-20
- 실행 시각(UTC): 2026-09-20T11:56:06.874498+00:00
- 기준 커밋: `0c1e0ddd0caa43120c04fe0a59d8c591301cb193`
- 미커밋 변경 포함: True
- 환경: ASGI TestClient + disposable mysql:8.4; external network blocked; synthetic users only
- 백엔드 소스·테스트 SHA-256: `a2c1c024c0f7e1b6d54cdabd4c554d4cb58cf324720f6826afa530bfd188ce72`
- 결과: {'PASS': 38, 'OPEN': 20, 'ERROR': 0, 'SKIP': 0}
...
| test_spot_routes_reject_anonymous_impersonation[create] | critical | 서명된 세션 없이 사용자 번호만 보내면 401, 저장·삭제 없음 | HTTP 201; spots=2 | OPEN |
| test_spot_routes_reject_anonymous_impersonation[delete] | critical | 서명된 세션 없이 사용자 번호만 보내면 401, 저장·삭제 없음 | HTTP 204; deleted=True | OPEN |
| test_spot_routes_reject_anonymous_impersonation[mine] | critical | 서명된 세션 없이 사용자 번호만 보내면 401, 저장·삭제 없음 | HTTP 200 | OPEN |
| test_spot_routes_reject_anonymous_impersonation[comment] | critical | 서명된 세션 없이 사용자 번호만 보내면 401, 저장·삭제 없음 | HTTP 201; comments=1 | OPEN |
| test_spot_routes_reject_anonymous_impersonation[like] | critical | 서명된 세션 없이 사용자 번호만 보내면 401, 저장·삭제 없음 | HTTP 200; reactions=1 | OPEN |
| test_spot_routes_reject_anonymous_impersonation[upload] | critical | 서명된 세션 없이 사용자 번호만 보내면 401, 저장·삭제 없음 | HTTP 201 | OPEN |
...
| test_signed_in_user_cannot_delete_another_users_spot | critical | 사용자 2의 세션에 사용자 1 헤더를 넣어도 타인 글 삭제 불가 | HTTP 204; deleted=True | OPEN |
| test_pending_comment_cannot_be_read_by_spoofing_owner | high | 비로그인 조회에서 헤더를 바꿔도 미승인 댓글 비공개 | HTTP 200; pending_comment_disclosed=True | OPEN |
...
| test_demo_flag_requires_server_authorization | medium | 일반 사용자 요청의 demoCompletion만으로 위치 검증 생략 불가 | HTTP 200; reward_rows=1 | OPEN |
| test_far_away_quest_rejected | medium | 먼 거리의 일반 완료 요청은 보상 없이 거부 | HTTP 400 | PASS |
